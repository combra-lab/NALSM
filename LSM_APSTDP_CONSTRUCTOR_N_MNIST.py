import numpy as np
import time
import NALSM_LIF_v2 as neuron
import LSM_APSTDP_STDP as plasticity
...
class simulation_constructor:
    def __init__(self
        ...
        self.run_sup = runSupport.run_support(number_of_networks_IN=BATCH_SIZE_IN)
        ...
        if INITIALIZE_STDP == True:
            stdp = plasticity.stdp(stdp_LR_fixed_potentiation=STDP_POTENTIATION_LR_IN
                                   ...
                                   , LTP_C_max=STDP_LTP_C_MAX_IN
                                   , LTP_C_min=STDP_LTP_C_MIN_IN
                                   , LTD_C_max=STDP_LTD_C_MAX_IN
                                   , LTD_C_min=STDP_LTD_C_MIN_IN
                                   )
        ...
        ini_names, ini_data = self.run_sup.simulation_initializer(inp_range_IN=self.inp_range
        ...
        if INITIALIZE_STDP == True:
            ...
            self.C_RES = stdp_data[stdp_names.index('C_RES')]
        ...
        # STDP OPS
        if INITIALIZE_STDP == True:
            self.PLASTICITY_update_stdp_only_op = stdp.APSTDP(
                                                                W_dense_IN=self.W_DENSE
                                                                , S_IN=self.S
                                                                , C_RES_IN=self.C_RES
            ...
            self.PLASTICITY_update_C_VAR_op = stdp.update_calcium_rate_measures(TAU_C_IN=TAU_C_VAL, C_RES_IN=self.C_RES,
                                                                            S_IN=self.S)
    ...
    def generate_validation_or_test_spike_data_STATIC(self
        ...
        '''
        GENERATES VALIDATION/TEST SPIKE COUNT DATA FOR LSM+APSTDP MODEL

        :param sess_IN: TF SESSION
        :param saver_object_IN: OJECT FOR SAVING DATA
        :param valid_or_test_data_idx_l_IN: INPUT DATA INDEXS
        ...
        :param data_type_IN: DATA PARTITION (TRAIN, VALIDATION, TEST)
        :return: 
        '''
        ...
        for ITER in range(0, ITERATIONS):
            ...
            for bt in range(0, BATCH_SIZE_IN):
                ...
                valid_or_test_data_INPUT_VEC, valid_or_test_data_LABEL_VEC, valid_or_test_data_LABEL_SCALAR = self.run_sup.get_nmnist_data_point(idx_IN=sample_idx,data_type_IN=str(data_type_IN),data_list_IN=valid_or_test_data_idx_l_IN,INPUT_DURATION_IN=SAMPLE_INPUT_DURATION_MS_IN)

                batch_samples_agg_temp.append(valid_or_test_data_INPUT_VEC)
                target_output_for_linear_layer_labels_in_batch_l.append(valid_or_test_data_LABEL_VEC)
                scalar_labels_in_batch_l.append(valid_or_test_data_LABEL_SCALAR)
            ...
            if ITER % spike_store_batch_mod_IN == spike_store_batch_mod_IN-1:
                for bt_s in range(0,len(record_samples_in_batch_l_IN)):
                    ...
                    scalar_labels_in_batch_for_spike_data_l.append(valid_or_test_data_idx_l_IN[sample_idx][1])

            batch_samples_agg_arr = np.asarray(batch_samples_agg_temp)
            ...
            for t in range(0, SAMPLE_INPUT_DURATION_MS_IN):
                ...
                sess_IN.run(self.LIF_update_neuron_states_op, feed_dict={self.I_PH:batch_samples_agg_arr[:,:,t]})
    ...
    def train_on_set_number_of_batches_STATIC(self
        ...
        THIS FUNCTION USED FOR LSM+APSTDP MODEL, WITH STATIC/FIXED WEIGHTS
        ...
        if len(iters_with_no_existing_data_l)>0:
            ...
            for ITER in iters_with_no_existing_data_l:
                ...
                for bt in range(0, BATCH_SIZE_IN):
                    ...
                    train_data_INPUT_VEC, train_data_LABEL_VEC, train_data_LABEL_SCALAR = self.run_sup.get_nmnist_data_point(
                        idx_IN=sample_idx, data_type_IN=str(data_type_IN), data_list_IN=train_data_idx_l_IN,INPUT_DURATION_IN=SAMPLE_INPUT_DURATION_MS_IN)

                    batch_samples_agg_temp.append(train_data_INPUT_VEC)
                    target_output_for_linear_layer_labels_in_batch_l.append(train_data_LABEL_VEC)
                    scalar_labels_in_batch_l.append(train_data_LABEL_SCALAR)


                if ITER % spike_store_batch_mod_IN == spike_store_batch_mod_IN - 1:
                    for bt_s in range(0, len(record_samples_in_batch_l_IN)):
                        sample_idx = ((ITER * BATCH_SIZE_IN) + bt_s)
                        scalar_labels_in_batch_for_spike_data_l.append(train_data_idx_l_IN[sample_idx])
    ...
    def run_W_DENSE_initialization_loop(self
        ...
        '''
        RUNS INITIALIZATION OF LIQUID WEIGHTS BY PRESENTING SERIES OF SNAPSHOTS TO LIQUID WITH APSTDP
        ...
        original_start_time_code = START_TIME_MS_IN
        ...
        for INI_ITER in range(0,INI_NUM_TRAIN_VAL_TEST_L_IN[0]):
            ...
            for bt in range(0, INI_BATCH_SIZE_IN):
                ...
                file_number = int(INI_initialization_idx_l[sample_idx])

                train_data_INPUT_VEC, train_data_LABEL_VEC, train_data_LABEL_SCALAR = self.run_sup.get_nmnist_data_point(
                    idx_IN=file_number, data_type_IN='train_all',data_list_IN=train_data_idx_l_IN,INPUT_DURATION_IN=300)


            train_data_INPUT_VEC_exp = np.expand_dims(train_data_INPUT_VEC,axis=0)
            ...
            if original_start_time_code == -1:
                START_TIME_MS_IN = np.random.randint(1, 300 - 1 - INI_SAMPLE_INPUT_DURATION_MS_IN)
            ...
            for t in range(0,INI_SAMPLE_INPUT_DURATION_MS_IN):
                ...
                sess_IN.run(self.PLASTICITY_update_C_VAR_op)

                sess_IN.run(self.LIF_propagate_spike_op)
                sess_IN.run(self.LIF_update_neuron_states_op, feed_dict={self.I_PH:train_data_INPUT_VEC_exp[:,:,START_TIME_MS_IN+t]})
                sess_IN.run(self.LIF_evolveCurr_op)
                sess_IN.run(self.LIF_update_refractory_state_op)
                ############### MAIN LIF FUNCTIONS ###################### END
