...
class simulation_constructor:
    ...
    def generate_validation_or_test_spike_data_PURE_STDP(self
        ...
        for ITER in range(0, ITERATIONS):
            ...
            for bt in range(0, BATCH_SIZE_IN):
                ...
                valid_or_test_data_INPUT_VEC, valid_or_test_data_LABEL_VEC, valid_or_test_data_LABEL_SCALAR = self.run_sup.get_nmnist_data_point(
                    idx_IN=sample_idx, data_type_IN=str(data_type_IN), data_list_IN=valid_or_test_data_idx_l_IN,INPUT_DURATION_IN=SAMPLE_INPUT_DURATION_MS_IN)
    ...
    def generate_validation_or_test_spike_data_PURE_STDP_with_NOISE(self
                                               , sess_IN
                                               , saver_object_IN
                                               , valid_or_test_data_idx_l_IN
                                               , BATCH_SIZE_IN
                                               ...
                                               , root_save_name_IN
                                               , data_type_IN
                                                , TEMPORAL_NOISE_MAX_RANGE_IN
                                               ):

        '''
        GENERATES SPIKE COUNTS FOR LSM+STDP MODEL WITH ADDED ORIGINAL EVENTS TIME SHIFTED BY GAUSSIAN NOISE
        USED FOR KERNEL QUALITY EVALUATION ONLY
        ...
        :param record_samples_in_batch_l_IN: SAMPLES TO STORE SPIKE DATA FROM IN BATCH
        :param root_save_name_IN: SAVE NAME
        :param data_type_IN: WHAT PARTITION OF DATA TO DRAW FROM (TRAIN, VALIDATION, TEST)
        :param TEMPORAL_NOISE_MAX_RANGE_IN: VARIANCE FOR NOISE
        :return: 
        '''
        ...
        for ITER in range(0, ITERATIONS):
            ...
            for bt in range(0, BATCH_SIZE_IN):
                ...
                valid_or_test_data_INPUT_VEC, valid_or_test_data_LABEL_VEC, valid_or_test_data_LABEL_SCALAR = self.run_sup.get_nmnist_data_point_with_NOISE(
                    idx_IN=sample_idx, data_type_IN=str(data_type_IN), data_list_IN=valid_or_test_data_idx_l_IN,INPUT_DURATION_IN=SAMPLE_INPUT_DURATION_MS_IN,time_var_IN=TEMPORAL_NOISE_MAX_RANGE_IN)
